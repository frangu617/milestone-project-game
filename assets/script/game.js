...
function enemyOnEnemyColl() {
    //     for (let i = 0; i < enemies.length; i++) {
    //         for (let j = 0; j < enemies.length; j++) {
    //             if (collisionCheckRectRect(enemies[i], enemies[j])) {

    //                 enemies[i].x -= enemies[j].width;
    //                 enemies[i].y -= enemies[j].height;
    //             }
    //         }
    //     }
    // }
    for (let i = 0; i < enemies.length; i++) {
        for (let j = i + 1; j < enemies.length; j++) {
            ...
            // Check if enemies are colliding (adjust 2 to the sum of their radii)
            if (distance < enemyA.width / 2 + enemyB.width / 2) {
                // Handle the collision
                resolveEnemyCollision(enemyA, enemyB, dx, dy, distance);
            }
        }
    }
}
